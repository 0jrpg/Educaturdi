...
-- Admin tem acesso total: pode editar e excluir qualquer perfil
-- (necessário para a tela de Usuários funcionar de ponta a ponta)
-- Função auxiliar (SECURITY DEFINER) pra checar se o usuário é admin.
-- Importante: ela roda com privilégio elevado e por isso NÃO aciona a
-- Row Level Security de novo ao consultar profiles por dentro — se a
-- gente checasse "é admin?" com uma subquery comum direto na policy da
-- própria tabela profiles, o Postgres detecta isso como referência
-- circular e recusa a consulta ("infinite recursion detected in policy").
...
-- Professor/Admin têm acesso total às entregas (podem lançar, corrigir e
-- remover status de entrega em nome de qualquer aluno)
...
-- ════════════════════════════════════════════════════════════
--  TABELA: resumos
--  Resumos de aula (PDF ou Word) para consulta rápida dos alunos
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
--  STORAGE — bucket para upload de PDFs/DOCXs (resumos)
-- ════════════════════════════════════════════════════════════
insert into storage.buckets (id, name, public)
...
-- ════════════════════════════════════════════════════════════
--  STORAGE — bucket de anexos (foto) enviados pelo aluno junto com a
--  resposta de uma atividade. Fica só 7 dias e é apagado sozinho.
-- ════════════════════════════════════════════════════════════
insert into storage.buckets (id, name, public)
