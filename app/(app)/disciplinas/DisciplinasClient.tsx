...
export default function DisciplinasClient({ disciplinas }: { disciplinas: Disciplina[] }) {
  ...
  async function confirmarExclusao() {
    ...
    if (error) {
      showToast('Erro ao excluir — verifique se ela não está em uso em notas, atividades ou apostilas: ' + error.message, 'error');
      return;
    }
  ...
  return (
    <div>
      <div className="page-header">
        <div><h1>Disciplinas</h1><p>Matérias disponíveis para notas, atividades, apostilas e resumos</p></div>
      ...
      <div className="card">
        ...
        <div className="table-wrap">
          {disciplinas.length === 0 ? (
            <div className="empty-state"><h3>Nenhuma disciplina cadastrada</h3><p>Cadastre a primeira disciplina para liberar o lançamento de notas, atividades e materiais para ela.</p></div>
          ) : (
      ...
      >
        <p style={{ fontSize: 14, color: 'var(--s600)' }}>
          Tem certeza que deseja excluir <strong>{excluindo?.nome}</strong>? Isso só é possível se não houver notas, atividades ou materiais usando essa disciplina.
        </p>
      </Modal>
    </div>
  );
