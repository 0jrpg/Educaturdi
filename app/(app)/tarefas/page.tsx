...
import type { Tarefa, Disciplina, Turma, Profile } from '@/types/database';
...
export default async function TarefasPage() {
  ...
  const { data: tarefasRaw } = await supabase.from('tarefas').select('*').order('prazo');
  const tarefas = (tarefasRaw as Tarefa[] | null) ?? [];
  ...
  if (isAluno) {
    ...
    const { data: entregas } = await supabase.from('entregas').select('*, aluno:profiles(nome, turma)').order('entregue_em', { ascending: false });
    for (const e of (entregas as any[] | null) ?? []) {
      if (!respostasPorItem[e.tarefa_id]) respostasPorItem[e.tarefa_id] = [];
      respostasPorItem[e.tarefa_id].push(e);
    }
    const { data: alunosData } = await supabase.from('profiles').select('*').eq('tipo', 'aluno').order('nome');
    alunos = (alunosData as Profile[] | null) ?? [];
  }

  const itensComStatus = tarefas.map(t => ({
    ...t,
    status: isAluno
