...
import type { Tarefa, Comunicado, Disciplina, Horario, Nota } from '@/types/database';
import {
  IconReportAnalytics, IconClipboardList, IconCircleCheck, IconBooks,
  IconSpeakerphone, IconCalendar, IconFlag3,
...
export default async function DashboardPage() {
  ...
  // Tarefas relevantes (RLS já filtra por turma se for aluno)
  const { data: tarefasRaw } = await supabase.from('tarefas').select('*').order('prazo', { ascending: true });
  const minhasTarefas = (tarefasRaw as Tarefa[] | null) ?? [];

  let entregasMap = new Map<string, string>();
  if (isAluno) {
    const { data: entregas } = await supabase.from('entregas').select('*').eq('aluno_id', user!.id);
    entregasMap = new Map((entregas ?? []).map((e: any) => [e.tarefa_id, e.status]));
  }

  const tarefasComStatus = minhasTarefas.map(a => ({
    ...a,
    status: isAluno ? (entregasMap.get(a.id) ?? (diasAte(a.prazo) < 0 ? 'atrasado' : 'pendente')) : 'pendente',
  }));

  const pendentes = tarefasComStatus.filter(a => a.status === 'pendente').length;
  const atrasadas = tarefasComStatus.filter(a => a.status === 'atrasado').length;
  const entregues = tarefasComStatus.filter(a => a.status === 'entregue').length;
  ...
  let minhasFichas = 0;
  if (isAluno) {
    const { count } = await supabase.from('fichas').select('id', { count: 'exact', head: true }).eq('aluno_id', user!.id);
    minhasFichas = count ?? 0;
  }
  ...
  const pendentesParaExibir = tarefasComStatus.filter(a => a.status !== 'entregue').slice(0, 4);

  return (
    <div>
      <div style={{ marginBottom: '1.5rem' }}>
        <h1 style={{ fontSize: 25, fontWeight: 800, color: 'var(--s900)' }}>
      ...
      <div className="grid-4" style={{ marginBottom: '1.5rem' }}>
        ...
        {isAluno ? (
          <div className="stat-card fadeUp d3">
            ...
            <div className="stat-lbl">Fichas</div>
            <div className="stat-val">{minhasFichas}</div>
            <div className="stat-sub">{minhasFichas >= 3 ? <span style={{ color: 'var(--re700)' }}>Responsável será chamado</span> : 'Comportamento'}</div>
          </div>
        ) : (
          <div className="stat-card fadeUp d3">
      ...
      <div className="grid-2" style={{ gap: '1.4rem' }}>
        <div className="card fadeUp d2">
          <div className="card-hd">
            <h3><IconClipboardList size={16} /> Próximas Tarefas</h3>
            <Link href="/tarefas" className="btn btn-ghost btn-sm">Ver todas →</Link>
          </div>
            ...
            return (
              <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '11px 14px', borderBottom: '1px solid var(--s100)' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--s400)' }}>{a.disciplina} · {a.pontos.toFixed(1)} pts</div>
                </div>
