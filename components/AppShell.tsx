...
const NAV_GROUPS: { label: string; links: NavLink[] }[] = [
  {
    label: 'Principal',
    links: [
      { href: '/dashboard', label: 'Início', icon: <IconLayoutDashboard size={17} />, roles: ['aluno', 'professor', 'admin'] },
      { href: '/tarefas', label: 'Tarefas', icon: <IconClipboardList size={17} />, roles: ['aluno', 'professor', 'admin'] },
      { href: '/trabalhos', label: 'Trabalhos', icon: <IconBriefcase size={17} />, roles: ['aluno', 'professor', 'admin'] },
      { href: '/provas', label: 'Provas', icon: <IconClockHour4 size={17} />, roles: ['aluno', 'professor', 'admin'] },
      { href: '/apostilas', label: 'Apostilas', icon: <IconBooks size={17} />, roles: ['aluno', 'professor', 'admin'] },
      { href: '/resumos', label: 'Resumos', icon: <IconFileText size={17} />, roles: ['aluno', 'professor', 'admin'] },
      { href: '/notas', label: 'Notas', icon: <IconReportAnalytics size={17} />, roles: ['aluno', 'professor', 'admin'] },
      { href: '/comunicados', label: 'Comunicados', icon: <IconSpeakerphone size={17} />, roles: ['aluno', 'professor', 'admin'] },
      { href: '/horario', label: 'Horário', icon: <IconCalendar size={17} />, roles: ['aluno', 'professor', 'admin'] },
      { href: '/fichas', label: 'Fichas', icon: <IconFlag3 size={17} />, roles: ['aluno', 'professor', 'admin'] },
    ],
  },
  {
    label: 'Gestão',
    links: [
      { href: '/turmas', label: 'Turmas', icon: <IconUsers size={17} />, roles: ['professor', 'admin'] },
      { href: '/disciplinas', label: 'Disciplinas', icon: <IconBooks size={17} />, roles: ['admin'] },
      { href: '/usuarios', label: 'Usuários', icon: <IconUserCog size={17} />, roles: ['admin'] },
    ],
  },
  {
    label: 'Conta',
    links: [
      { href: '/perfil', label: 'Meu Perfil', icon: <IconUserCircle size={17} />, roles: ['aluno', 'professor', 'admin'] },
    ],
  },
];
...
export default function AppShell({
  ...
  function togglePinned() {
    setPinned((atual) => {
      const novo = !atual;
      window.localStorage.setItem(LS_KEY, novo ? '1' : '0');
      return novo;
    });
    setPeek(false);
  }
    ...
    <PresenceProvider userId={profile.id}>
      <div id="app-shell">
        ...
        <aside
          ...
          onMouseEnter={() => { if (!pinned) abrirPeek(); }}
        ...
        <div
          className={`sidebar-overlay ${sidebarOpenMobile ? 'open' : ''}`}
          onClick={() => setSidebarOpenMobile(false)}
        />
