...
export default function NovaApostilaModal({
  ...
  function limpar() {
    setTitulo(''); setDescricao(''); setTurmasSelecionadas([]); setPaginas('');
    setArquivo(null); setEmoji('📄'); setCodigoCdn(''); setProgresso(0);
  }
  ...
  async function salvar() {
    ...
    if (!arquivo && !codigoCdn.trim()) {
      return showToast('Envie um arquivo PDF ou informe o código do link da CDN.', 'error');
    }
    ...
    try {
      ...
      if (arquivo) {
        ...
        arquivoUrl = montarUrlCdn(`${codigoCdn.trim()}.pdf`);
        setProgresso(70);
      }
      ...
      await criarNotificacao(supabase, {
        tipo: 'apostila',
        titulo: `Nova apostila: ${titulo.trim()}`,
      ...
      <div className="fg">
        ...
        >
          ...
          <span
            style={{
              display: 'flex', alignItems: 'center', padding: '0 10px',
              color: 'var(--s500)', fontSize: 12.5, fontWeight: 600, whiteSpace: 'nowrap',
            }}
          >
            .pdf
          </span>
